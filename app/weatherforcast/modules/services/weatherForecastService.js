...
factory('weatherForecastService',['$http','$q',function($http,$q){
	...
	.success(function(data,status,headers,config){
		if(!data.query||!data.query.results||!data.query.results.place) fail(defer,"Please check the city and country and try again!!!");
		$http({
           method: 'JSONP',
           url: "http://query.yahooapis.com/v1/public/yql?q=select item from weather.forecast " +
            	...
            	if(data.query.results.channel.item.title=="City not found") fail(defer,"Multiple cities possible");
            	defer.resolve(data)
            }).error(function(){fail(defer,"Please check the city and country and try again!!!")});
		})
	.error(function(){fail(defer,"Please check the city and country and try again!!!")});
