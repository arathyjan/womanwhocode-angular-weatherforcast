	...
	var getWeatherForcastFor=function(city,country,unit){
		...
		$http({
			...
			function(data,status,headers,config){
				...
				$http({
           			method: 'JSONP',
           			url: "http://query.yahooapis.com/v1/public/yql?q=select item from weather.forecast where woeid='"+data.query.results.place.woeid+ "' and u='" + unit + "'&format=json&callback=JSON_CALLBACK"
